...
void main() {
  group('movement-group diversity (all levels)', () {
    test('fresh group +3, repeated group -15', () {
      ...
      final base = _scorer.scoreOf(
          fresh, _slot, stateWith([]), ExperienceLevel.beginner);
      ...
      final repeated =
          _scorer.scoreOf(fresh, _slot, used, ExperienceLevel.beginner);
  ...
  group('scoreBeginnerEquipment (none/beginner only)', () {
    test('Smith Machine +8 under cap, -50 at cap', () {
      final smith = ex('Smith Press',
          category: 'chest press', equipment: 'Smith Machine');
  ...
  group('scoreAdvancedEquipment (advanced only)', () {
    test('free-weight compound +10, isolation +4', () {
      final compound =
          ex('Bench', category: 'chest press', equipment: 'Barbell');
      final iso = ex('DB Fly',
          category: 'chest fly',
          equipment: 'Dumbbell',
      ...
      expect(
        _scorer.scoreOf(
            compound, _slot, stateWith([]), ExperienceLevel.advanced),
        10,
      );
      expect(
    ...
    test('free-weight diversity +5 when the missing type appears', () {
      final db = ex('DB Press',
          category: 'chest press', equipment: 'Dumbbell');
    ...
    test('machine compound -6, supported_row only -3', () {
      final machine = ex('Machine Press',
          category: 'chest press', equipment: 'Machine');
      final supportedRow = ex('Chest Supported Row',
          category: 'row',
          equipment: 'Machine',
      ...
      expect(
        _scorer.scoreOf(
            machine, _slot, stateWith([]), ExperienceLevel.advanced),
        -6,
      );
      ...
      expect(
        _scorer.scoreOf(
            supportedRow, _slot, stateWith([]), ExperienceLevel.advanced),
        0,
      );
    });

    test('machine/plate dominance guard -25 when ≥2 machines present', () {
      final machineIso = ex('Machine Fly',
          category: 'chest fly',
          equipment: 'Machine',
          type: ExerciseType.isolation);
      ...
      expect(
        _scorer.scoreOf(
            machineIso, _slot, twoMachines, ExperienceLevel.advanced),
        -25,
      );
    });
  });
  ...
  group('scoreShoulderCandidate (advanced only)', () {
    test('+12 free-weight press when none present, -10 machine', () {
      final dbPress = ex('DB Shoulder Press',
          category: 'shoulder press', equipment: 'Dumbbell');
      final machinePress = ex('Machine Shoulder Press',
          category: 'shoulder press', equipment: 'Machine');
      // Free-weight compound: +10 (advanced fw) +12 (shoulder) = 22.
      expect(
        _scorer.scoreOf(
            dbPress, _slot, stateWith([]), ExperienceLevel.advanced),
        22,
      );
      // Machine compound: -6 (machine compound) -10 (shoulder machine) = -16.
      expect(
        _scorer.scoreOf(
            machinePress, _slot, stateWith([]), ExperienceLevel.advanced),
        -16,
      );
    });
    ...
    test('no shoulder bonus once a free-weight press exists', () {
      ...
      expect(
        _scorer.scoreOf(
            machinePress, _slot, withFwPress, ExperienceLevel.advanced),
        -6,
      );
    });
  });
  ...
  group('scoreDipsCandidate (advanced only, "Dips" only)', () {
    Exercise dips() =>
        ex('Dips', category: 'triceps', equipment: 'Bodyweight');
    ...
    test('-100 when 3 press movements already present', () {
      ...
      expect(
        _scorer.scoreOf(
            dips(), _slot, threePresses, ExperienceLevel.advanced),
        -100,
      );
    });
    ...
    test('-50 when Close Grip Bench Press already selected', () {
      final withCgbp = stateWith([
        ex('Close Grip Bench Press',
            category: 'chest press', equipment: 'Barbell'),
      ]);
    ...
    test('compound press is NOT counted as a press (matches prototype)', () {
      ...
      expect(
        _scorer.scoreOf(
            dips(), _slot, threeCompound, ExperienceLevel.advanced),
